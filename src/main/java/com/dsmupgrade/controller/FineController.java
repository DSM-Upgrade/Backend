...
public class FineController {
    ...
    public List<Fine> getUser_FineList(@PathVariable("username") String username){ //유저의 따른 벌금리스트를 받아옴 (아직 null 처리 안함 + 형식이 안 맞음)
        List<Fine> user_fineList =  fineRepository.findAllByUsername(username);
        return user_fineList;
    }
    @PostMapping("imposition")
    public void imposeFine(@RequestBody ImpositionRequest impositionRequest){ // 유저에 따른 벌금 부과 (아직 null 처리 안함 + 토큰 확인 안함)
        Fine fine = new Fine();
    ...
    public void completeFine(@RequestBody CompletionFine completionFine){ // 유저가 벌금을 냄 (아직 null 처리 안함 + 토큰 확인 안함)
